...
import { APP_DOMAIN, cn, createHref, fetcher } from "@leadswap/utils";
...
export function NavMobile({
  ...
  let { domain = "cliqo.com" } = useParams() as { domain: string };
  if (staticDomain) {
    domain = staticDomain;
  }
  ...
  const { data: session, isLoading } = useSWR(
    domain.endsWith("cliqo.com") && "/api/auth/session",
    fetcher,
    {
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            ...
            className="fixed inset-0 z-20 max-h-screen w-full overflow-y-auto bg-white px-5 py-16 lg:hidden dark:bg-black dark:text-white/70"
          >
...
export function AuthButton({
  ...
  className,
  ...rest
}: Pick<ButtonProps, "variant" | "className"> & ComponentProps<typeof Link>) {
  return (
    // @ts-ignore
    <Link
      {...rest}
