...
const navigation = {
  product: [
    ...FEATURES_LIST.filter(({ title }) => title !== "Cliqo Integrations").map(
      ({ id, title, href }) => ({
        id,
        name: title,
        href,
      ...
      <footer>
                ...
                <ul role="list" className={linkListClassName}>
                  {navigation.resources.map((item) => (
                    <li key={item.name}>
                      ...
                      >
                        {item.name}
                        {(item as { target?: string }).target && <ReferredVia className="size-3.5" />}
                      </Link>
                    </li>
                  ))}
                </ul>
        ...
        <div className="mt-12 grid grid-cols-1 items-center gap-8 sm:grid-cols-3">
          ...
          <p className="text-xs text-neutral-500 sm:text-right">
            © {new Date().getFullYear()} Cliqo Technologies, Inc.
          </p>
        </div>
      </footer>
    </MaxWidthWrapper>
