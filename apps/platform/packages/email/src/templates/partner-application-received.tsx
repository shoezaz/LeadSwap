import { CLIQO_WORDMARK, OG_AVATAR_URL } from "@leadswap/utils";
import {
  Body,
  Container,
  Head,
...
export default function CreatorApplicationReceived({
  ...
  const applicationUrl = `https://app.cliqo.com/${workspace.slug}/program/creators/applications?creatorId=${creator.id}`;

  return (
    <Html>
      <Head />
      <Preview>
        {creator.name} ({creator.email}) just applied to your creator program,{" "}
        ...
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[600px] rounded border border-solid border-neutral-200 px-10 py-5">
            ...
            <Heading className="mx-0 p-0 text-lg font-medium text-neutral-800">
              New creator application for {program.name}
            </Heading>

            <Text className="text-sm leading-6 text-neutral-600">
              Your program <strong>{program.name}</strong> just received a new
              application from <strong>{creator.name}</strong> ({creator.email}
            ...
            {program.autoApproveCreators && (
              <Text className="text-sm leading-6 text-neutral-600">
                Since you have auto-approve creators enabled, this application
            ...
            <Container className="mb-8 mt-10 rounded-lg border border-solid border-neutral-200">
              <Section className="p-2">
                <Container className="mb-4 w-full rounded-lg border border-solid border-neutral-100 bg-neutral-50 p-6">
                  <div>
                    <div className="relative w-fit">
                      <Img
                        src={creator.image || `${OG_AVATAR_URL}${creator.name}`}
                        width="48"
                        height="48"
                        alt={creator.name}
                      ...
                      {creator.country && (
                        <div className="absolute -right-1 top-0 overflow-hidden rounded-full bg-neutral-50 p-0.5">
                          <Img
                            src={`https://flag.vercel.app/m/${creator.country}.svg`}
                            width="12"
                            height="12"
                            alt={creator.country}
                ...
                <Section className="p-4">
                  {creator.applicationFormData.map((field) => (
            ...
            <Footer
              email={email}
              notificationSettingsUrl={`https://app.cliqo.com/${workspace.slug}/settings/notifications`}
            />
          </Container>
        </Body>
      </Tailwind>
