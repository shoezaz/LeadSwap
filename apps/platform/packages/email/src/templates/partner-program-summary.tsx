...
const ICONS = {
  clicks: "/assets/misc/icons/nucleo/cursor-rays.png",
  leads: "/assets/misc/icons/nucleo/user-plus.png",
  sales: "/assets/misc/icons/nucleo/invoice-dollar.png",
  earnings: "/assets/misc/icons/nucleo/money-bills.png",
} as const;
...
export default function CreatorProgramSummary({
  program = {
    name: "Acme",
    logo: CLIQO_LOGO,
    slug: "acme",
        ...
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[600px] space-y-10 px-3 py-5">
            <Section className="mt-8">
              <Img src={CLIQO_WORDMARK} height="32" alt={program.name} />
            </Section>

            <Heading className="mx-0 mt-[40px] p-0 text-lg font-medium text-black">
              {program.name} creator program monthly summary (
            ...
            <Section className="mt-6 rounded-xl border border-solid border-neutral-200 bg-neutral-50">
              <Section className="rounded-t-xl px-6 py-5">
                <div className="flex items-center">
                  <Img
                    src={program.logo || CLIQO_WORDMARK}
                    alt={program.name}
                    height="32"
                    width="32"
                  ...
                  <div>
                    ...
                    <div className="text-xs font-medium text-neutral-500">
                      Creator since{" "}
                      {formatDate(creator.createdAt, { month: "short" })}
                    </div>
                  </div>
                </div>
              </Section>
              ...
              <Section className="space-y-6 rounded-xl border-t border-solid border-neutral-200 bg-white p-6">
                ...
                <Section className="mt-8 text-center">
                  <Link
                    href={`${PARTNERS_DOMAIN}/programs/${program.slug}?start=${reportingPeriod.start}&end=${reportingPeriod.end}`}
