...
import { useLocalStorage } from "@leadswap/ui";
...
export default function AnalyticsProvider({
  ...
  const { basePath, domain, baseApiPath, eventsApiPath } = useMemo(() => {
    if (adminPage) {
      ...
      return {
        basePath: `/api/partner-profile/campaigns/${programSlug}/analytics`,
        baseApiPath: `/api/partner-profile/campaigns/${programSlug}/analytics`,
        eventsApiPath: `/api/partner-profile/campaigns/${programSlug}/events`,
        domain: domainSlug,
      };
    } else if (dashboardId) {
      // Public stats page, e.g. app.cliqo.com/share/dsh_123
      return {
  ...
  return (
    <AnalyticsContext.Provider
      value={{
        ...
        view,
        queryString,
        domain: domain || undefined, // domain for the link (e.g. cliqo.com, stey.me, etc.)
