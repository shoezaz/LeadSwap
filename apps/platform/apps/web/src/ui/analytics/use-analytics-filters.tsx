...
import { useWorkspaceBounties } from "@/lib/swr/use-workspace-bounties";
...
export function useAnalyticsFilters({
  ...
  const { plan, id: workspaceId, exceededAI } = useWorkspace();
  ...
  const { bounties, isLoading: loadingBounties } = useWorkspaceBounties();
  ...
  const CustomerFilterItem = {
    ...
    getOptionPermalink: () => {
      return programSlug
        ? `/campaigns/${programSlug}/customers/${selectedCustomerId}`
        : slug
  ...
  const filters: ComponentProps<typeof Filter.Select>["filters"] = useMemo(
    () => [
            ...
            ]),
      {
        ...
        icon: Megaphone,
        label: "Campaign",
        shouldFilter: true, // Always allow filtering
        options: loadingBounties
          ? null
          : bounties?.map((bounty) => ({
              value: bounty.id,
              label: bounty.type.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()) + ` (${nFormatter(bounty.amount, { full: true })} ${bounty.currency})`,
              icon: <Megaphone className="h-4 w-4" />,
              right: undefined, // Add total stats if available
            })) ?? null,
      },
      {
        key: "country",
  ...
  const onSelect = useCallback(
    async (key, value) => {
      if (key === "ai") {
        ...
        try {
          ...
          const { object } = await generateFilters({ workspaceId, prompt });
          ...
          toast.error(
            e instanceof Error
              ? e.message
              : "Failed to generate analytics filters.",
          );
        } finally {
          setStreaming(false);
        }
    ...
    [queryParams, activeFilters, selectedTagIds, workspaceId, exceededAI],
  );
