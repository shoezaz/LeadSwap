...
import { CursorRays, InvoiceDollar, UserCheck } from "@leadswap/ui/icons";
...
export function AnalyticsCard<T extends string>({
  ...
  className?: string;
  eventOverride?: EventType | string;
}) {
  const { selectedTab: contextEvent } = useContext(AnalyticsContext);
  const event = eventOverride || contextEvent;
