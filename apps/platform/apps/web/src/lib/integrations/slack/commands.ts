...
import { APP_DOMAIN, SLACK_INTEGRATION_ID } from "@leadswap/utils";
import { z } from "zod";
...
export const handleSlashCommand = async (req: Request) => {
  ...
  // Find Cliqo user matching the Slack user profile
      ...
      blocks: [
        {
          ...
          text: {
            type: "plain_text",
            text: "Unable to find Cliqo account matching your Slack account. Only Cliqo users can use this command.",
          },
        },
      ],
    };
...
// Find Cliqo user for the given Slack user
    ...
    blocks: [
      ...
      },
      {
        ...
        elements: [
          {
            type: "mrkdwn",
            text: `*Created by* ${user.name} | ${createdAtDate} | <${APP_DOMAIN}/${workspace.slug}/campaigns | View on Cliqo>`,
          },
        ],
      },
    ],
